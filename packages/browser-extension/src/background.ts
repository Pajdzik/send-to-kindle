...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId === 'sendToKindle' && tab?.id) {
    try {
      // Get configuration
      const config = await chrome.storage.sync.get(['kindleEmail', 'workerUrl']);
      ...
      if (response.success) {
        ...
        chrome.notifications.create({
          type: 'basic',
          iconUrl: 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><text y="14" font-size="16">📚</text></svg>',
