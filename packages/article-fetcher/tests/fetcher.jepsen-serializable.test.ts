...
describe('Jepsen Serializable Article Test', () => {
  ...
  it('should extract content from complete jepsen.io serializable consistency article', () => {
    const html = readFileSync(join(__dirname, 'test-articles', 'jepsen.html'), 'utf-8');
    ...
    expect(result.content).toContain('serializability means that transactions appear to have occurred in');
